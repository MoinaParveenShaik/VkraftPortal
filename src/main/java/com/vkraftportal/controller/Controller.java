		...
		from("direct:processTimesheet").log("Timesheet : ${body}").process(new Processor() {
			...
			public void process(Exchange exchange) throws Exception {
				...
				if (services.timesheetExists(timesheetDataToSave)) {
					...
					timesheetDataToSave.setNewStatus("Submitted");
					timesheetDataToSave.setComments("No Comments Yet");
...
// ------------------------------------------------9. GetApprovedTimesheets HR Portal Timesheet------------------------------------------------------------
		...
		rest().get("/getApprovedTimesheeets").param().name("month").type(RestParamType.query).endParam().param().name("year")
		.type(RestParamType.query).endParam().param().name("newStatus").type(RestParamType.query).endParam().to("direct:approvedTimesheeets");
		from("direct:approvedTimesheeets").process(exchange -> {
			String month = exchange.getIn().getHeader("month", String.class);
			String year = exchange.getIn().getHeader("year", String.class);
			String newStatus = exchange.getIn().getHeader("newStatus", String.class);
			List<Timesheet> approvedEmployees = services.getEmployeesByMonthAndYearAndNewStatus(month,year, newStatus);
...
//		--------------------------------43. Get Employee By ProjectName---------------------------
		...
		from("direct:getListOfEmployeesInProject").process(exchange -> {
			...
			System.out.println("inside listOfEmployeesForParticularProject");
...
//				------------------59. remainder with month and year for pending employees-----------------
		...
		rest().get("/deleteTimesheet").param().name("employeeNumber").type(RestParamType.query).endParam().param()
				.name("month").type(RestParamType.query).endParam().param().name("year").type(RestParamType.query)
				.endParam().to("direct:deleteTS");
		from("direct:deleteTS").process(exchange -> {
			String employeeNumber = exchange.getIn().getHeader("employeeNumber", String.class);
			String month = exchange.getIn().getHeader("month", String.class);
			String year = exchange.getIn().getHeader("year", String.class);
			boolean timesheet = services.deleteByEmployeeNumberAndName(employeeNumber, month, year);
			if (timesheet) {
				exchange.getMessage().setBody("Status: Record for Employee " + employeeNumber + " for Month " + month
						+ " and Year " + year + " Deleted");
			} else {
...
//	 ---------------56. Approve Timesheet Employee Portal----------------------

//		rest().post("/approveTimesheet").param().name("employeeNumber").type(RestParamType.query).endParam()
//				.to("direct:approveTimesheet");
//		from("direct:approveTimesheet").process(exchange -> {
//			String employeeNumber = exchange.getIn().getHeader("employeeNumber", String.class);
//			Timesheet empInfo = services.findByEmpNumber(employeeNumber);
//			String month = empInfo.getMonth();
...
//				exchange.getMessage().setBody(empInfo);
//				exchange.getMessage().setHeader(Exchange.HTTP_RESPONSE_CODE, 200);
//			}
//		});
...
//	 --------------9. GetApprovedTimesheets HR Portal Timesheet-------------------
		...
		from("direct:approvedList").process(exchange -> {
			List<Timesheet> approvedEmployees = services.getEmployeeByStatus("Approved");
			if (approvedEmployees != null && !approvedEmployees.isEmpty()) {
				exchange.getMessage().setBody(approvedEmployees);
				exchange.getMessage().setHeader(Exchange.HTTP_RESPONSE_CODE, 200);
			} else {
...
//	----------------------42. Get Employee By Month, Year--------------

		rest().get("/listForProjectMonthYear").param().name("projectName").type(RestParamType.query).endParam().param()
				.name("month").type(RestParamType.query).endParam().param().name("year").type(RestParamType.query)
				.endParam().to("direct:getListByProjectNameMonthYear");
		from("direct:getListByProjectNameMonthYear").process(exchange -> {
			String projectName = exchange.getIn().getHeader("projectName", String.class);
			String month = exchange.getIn().getHeader("month", String.class);
			String year = exchange.getIn().getHeader("year", String.class);
			List<Timesheet> employeeInfo = services.findByProjectNameMonthYear(projectName, month, year);
			System.out.println(employeeInfo);
			exchange.getMessage().setBody(employeeInfo);
			exchange.getMessage().setHeader(Exchange.HTTP_RESPONSE_CODE, 200);
		});

//-----------------approved button mail------------------------------
		rest().post("/approveTimesheetMail").param().name("employeeNumber").type(RestParamType.query).endParam().param()
				.name("month").type(RestParamType.query).endParam().param().name("year").type(RestParamType.query)
				.endParam().param().name("projectName").type(RestParamType.query).endParam().param().name("comments")
				.type(RestParamType.query).endParam().to("direct:approveTimesheetMail");
		...
		from("direct:approveTimesheetMail").process(exchange -> {
			...
			String year = exchange.getIn().getHeader("year", String.class);
			String projectName = exchange.getIn().getHeader("projectName", String.class);
			String comments = exchange.getIn().getHeader("comments", String.class);
			Timesheet empInfo = services.findByEmpNumber(employeeNumber, month, year, projectName);
			System.out.println("indside approveTimesheetMail");
...
//			String projctName = timesheet.getProjectName();
			...
			if (empInfo != null) {
				...
				if ("TeamLead".equals(employeRole) && empRole != null && empRole.getEmail() != null) {
		...
		from("direct:rejectTimesheetMail").process(exchange -> {
			...
			String comments = exchange.getIn().getHeader("comments", String.class);
			Timesheet empInfo = services.findByEmpNumber(employeeNumber, month, year, projectName);
			RegisterEmployee employeeByEmployeeNumber = services
					.findEmployeeByEmployeeNumber(empInfo.getEmployeeNumber());
			System.out.println(empInfo);
			empInfo.setComments(comments);
			empInfo.setNewStatus("Rejected");
			System.out.println("inside rejectTimesheetMail" + comments);
			services.saveTimesheet(empInfo);
			String emailBody = services.emailBodyForRejected(empInfo);
			exchange.getMessage().setBody(emailBody);
			exchange.getMessage().setHeader("To", employeeByEmployeeNumber.getEmail());
			exchange.getMessage().setHeader(Exchange.CONTENT_TYPE, "text/plain");
			exchange.getMessage().setHeader(Exchange.HTTP_RESPONSE_CODE, 200);

			ProducerTemplate producerTemplate = exchange.getContext().createProducerTemplate();
			producerTemplate.sendBodyAndHeaders("direct:sendingRejectionMail", exchange.getMessage().getBody(),
					exchange.getMessage().getHeaders());

		});
		from("direct:sendingRejectionMail").setHeader("Subject", simple("Timesheet Rejection")).process(exchange -> {
			String recipientEmail = exchange.getMessage().getHeader("recipientEmail", String.class);
			String emailBody = exchange.getIn().getBody(String.class);
			exchange.getMessage().setBody(emailBody);
		}).toD("smtps://smtp.gmail.com:465?username=vaibhavilandge97@gmail.com&password=bjdo uoqc vmhc hwef&to=${header.recipientEmail}");
