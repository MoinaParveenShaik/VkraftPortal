...
public class Controller extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		from("direct:sendingMail").setHeader("Subject", simple("${header.emailSubject}"))
		.process(exchange -> {
			String recipientEmail = exchange.getMessage().getHeader("recipientEmail", String.class);
			String emailBody = exchange.getIn().getBody(String.class);
		...
		.name("status").type(RestParamType.query).endParam().to("direct:selectedForTechnicalOne");

		from("direct:selectedForTechnicalOne").process(exchange -> {
			String email = exchange.getIn().getHeader("email", String.class);
		...
		rest().post("/selectedForScreeningRound").param().name("email").type(RestParamType.query).endParam().param()
		...
		//-------------------------After Clicking on Reject It should delete record and send mail--------------

		rest().delete("/deleteScreeningRecord").param().name("email").type(RestParamType.query).endParam().
		to("direct:deleteFromScreeningRound");

		from("direct:deleteFromScreeningRound").process(exchange ->{
			String email=exchange.getIn().getHeader("email",String.class);
			AppliedCandidateInformation candidateInfo=services.findByEmail(email);
			services.deleteCandidateFromAppliedCandidateInformation(candidateInfo.getEmail());
			String emailBody = services.getRegretScreeningEmailBody(candidateInfo);
			String subject="Regret Mail";
		...
		from("direct:sendingRegretMail").setHeader("Subject",simple("${header.emailSubject}"))
		.process(exchange -> {
			String recipientEmail = exchange.getMessage().getHeader("recipientEmail", String.class);
			String emailBody = exchange.getIn().getBody(String.class);
